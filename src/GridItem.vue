...
import type { GridItemClasses, GridItemPosition, Inner } from './types/components'
...
const emit = defineEmits(['container-resized', 'resize', 'resized', 'move', 'moved', 'drag-event', 'resizeEvent'])
...
const handleDrag = (event: any): void => {
  if (props.static || isResizing.value)
    return
...
const handleResize = (event: any): void => {
  ...
  emitter?.emit('resizeEvent', [event.type, props.i, inner.value.x, inner.value.y, pos.h, pos.w])
}
...
onMounted(() => {
  if (props.lastBreakpoint)
    cols.value = getColsFromBreakpoint(props.lastBreakpoint, props.breakpointCols) as number
