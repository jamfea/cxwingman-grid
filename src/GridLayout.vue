...
import type { Events } from './types/symbols'
...
watch(() => props.responsive, (value) => {
  if (!value) {
    ...
    emitter.emit('setColNum', props.colNum as number)
  }
  onWindowResize()
})
...
const layoutItemOptional = (props: { [key: string]: any }) => {
  ...
  return (Object.keys(props) as (keyof typeof props)[])
    .reduce((acc, val) => {
      if (layoutItemOptionalKeys.includes(val as string) || requiredKeys.includes(val as string))
        acc[val] = props[val]
...
const responsiveGridLayout = (): void => {
  ...
  emitter.emit('setColNum', getColsFromBreakpoint(newBreakpoint, props.cols) as number)
}
const onCreated = () => {
